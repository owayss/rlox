...
impl Environment {
    ...
    pub fn get(&self, t: Token) -> Result<Value, RuntimeErr> {
        if let Some(val) = self.globals.get(&t.lexeme) {
            ...
            Err(RuntimeErr::UndefinedSymbol(t.lexeme))
        }
    }
    ...
    pub fn assign(&mut self, name: String, value: Value) -> Result<Value, RuntimeErr> {
        if let Some(val) = self.globals.get_mut(&name) {
            *val = value;
            ...
            Err(RuntimeErr::AssignmentToUndefined(name))
        }
    }
}
